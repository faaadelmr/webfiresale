// src/app/admin/users/UserListClient.tsx
'use client';
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (result.success && result.users) {
          // Transform date objects to strings and handle nullable/undefined fields to match the User type
    ...
    try {
      ...
      if (result.success) {
        ...
        if (refreshResult.success && refreshResult.users) {
          // Transform date objects to strings and handle nullable/undefined fields to match the User type
          const transformedUsers = refreshResult.users.map(user => ({
            ...user,
            deletedAt: user.deletedAt ? new Date(user.deletedAt).toISOString() : null,
            createdAt: new Date(user.createdAt).toISOString(),
            updatedAt: new Date(user.updatedAt).toISOString(),
